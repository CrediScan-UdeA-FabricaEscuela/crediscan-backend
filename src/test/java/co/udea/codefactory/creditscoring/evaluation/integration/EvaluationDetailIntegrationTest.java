...
/**
 * Tests de integración para el detalle de una evaluación.
 * Verifica: id desconocido → 404; campos completos; colisión de path.
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class EvaluationDetailIntegrationTest {
    ...
    @Test
    void detalle_idInexistente_retorna404() throws Exception {
        String randomId = UUID.randomUUID().toString();
        mockMvc.perform(get(BASE_URL + "/" + randomId + "/detalle")
                        .with(user("admin").roles("ADMIN")))
                .andExpect(status().isNotFound());
    }

    @Test
    void detalle_evaluacionExistente_retornaCamposCompletos() throws Exception {
        crearVariables();
        String modeloId = crearYActivarModelo("Modelo Detalle");
        ...
        mockMvc.perform(get(BASE_URL + "/" + evaluacionId + "/detalle")
                        .with(user("admin").roles("ADMIN")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(evaluacionId))
                .andExpect(jsonPath("$.applicantId").value(aplicanteId))
                .andExpect(jsonPath("$.applicantName").isString())
                .andExpect(jsonPath("$.modelName").isString())
                .andExpect(jsonPath("$.modelVersion").isNumber())
                .andExpect(jsonPath("$.totalScore").isNumber())
                .andExpect(jsonPath("$.riskLevel").isString());
    }

    @Test
    void detalle_rutaClasificacionNoColisiona() throws Exception {
        // /evaluaciones/clasificacion NO debe ser interpretado como /{id}/detalle
        // Este test verifica que la ruta /clasificacion resuelve correctamente
    ...
    @Test
    void detalle_sinAutenticacion_retorna401() throws Exception {
        mockMvc.perform(get(BASE_URL + "/" + UUID.randomUUID() + "/detalle"))
